...
function LeadFormModal({ open, onClose }) {
    ...
    const [formData, setFormData] = useState({
        ...
        cargo: '', empresa: '', tempoLider: '', dores: '', origem: ''
    });
    ...
    const sendEmailEvent = async (eventType, recaptchaToken) => {
        try {
            const res = await fetch("/api/lead-notification", {
                ...
                body: JSON.stringify({ ...formData, event: eventType, recaptchaToken }),
            });
    ...
    useEffect(() => {
        if (open) {
            ...
            gsap.fromTo('.lead-modal-box', { y: 80, opacity: 0 }, { y: 0, opacity: 1, duration: 0.4, ease: 'power3.out' });
        }
    }, [open]);

    useEffect(() => {
        if (step === 4 && open) {
            gsap.fromTo('.success-icon', { scale: 0, y: 20, opacity: 0 }, { scale: 1, y: 0, opacity: 1, duration: 0.5, ease: 'back.out(1.7)' });
        }
    }, [step, open]);
            ...
            <div className="lead-modal-box main-bg-dark">
                ...
                {step === 1 && (
                    <div className="lead-step-webfolio">
                        ...
                        <input
                            className="lead-input" type="text" name="nascimento"
                            placeholder="Data de nascimento (DD/MM/AAAA)" maxLength={10}
                ...
                {step === 3 && (
                    <div className="lead-step-webfolio">
                        ...
                        <textarea className="lead-input" rows="4" name="dores" placeholder="Principais dúvidas ou dores na liderança" onChange={handleChange} />
                        <div className="lead-select-wrap">
                        ...
                        <div className="lead-actions-webfolio">
                            ...
                            <button
                                ...
                                onClick={async () => {
                                    ...
                                    await sendEmailEvent(TURMA_ABERTA ? "REGISTERED" : "WAITLIST", recaptchaToken);
                                    setStep(4);
                                }}
                            >
                ...
                {step === 4 && TURMA_ABERTA && (
                    <div className="lead-step-success">
                        ...
                        <p className="p1 mb-30">Sua inscrição foi registrada. Agora finalize sua vaga realizando o pagamento.</p>
                        <button className="butn butn-md radius-30 w-100 success-pay-btn mb-15" onClick={async () => { await sendEmailEvent('PIX_SELECTED'); setOpenPix(true); }}>
                            💙 Pagar no PIX (10% de desconto)
                        </button>
                        <a href="https://www.asaas.com/c/ihrq2yhz4ux3kcah" target="_blank" rel="noopener noreferrer" className="butn butn-md radius-30 w-100 butn-outline-blue mb-20" onClick={() => sendEmailEvent('CARD_SELECTED')}>
                ...
                {step === 4 && !TURMA_ABERTA && (
                    <div className="lead-step-success">
                        ...
                        <p className="p1 mb-30">
                            Assim que as vagas da próxima turma estiverem abertas, você será uma das primeiras pessoas a saber — antes de qualquer divulgação pública. Fique de olho no seu e-mail e nas nossas redes sociais! 🎉
                        </p>
                        <button className="butn butn-md radius-30 w-100 success-pay-btn mb-15" onClick={onClose}>
                            Perfeito, aguardo novidades!
                        </button>
                        <a href="https://wa.me/5511981017106" target="_blank" rel="noopener noreferrer" className="success-whatsapp-link">
                ...
                <div className="pix-modal">
                    ...
                    <div className="pix-modal-box main-bg-dark">
                        ...
                        <div className="mobile-only pix-copy-wrap">
                            ...
                            <button className="butn butn-md radius-30 w-100 success-pay-btn mt-15" onClick={handleCopyPix}>
